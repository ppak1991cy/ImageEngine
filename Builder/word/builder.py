...
from Builder.word.selector import WordSelector
...
class WordPictureInfo(PictureInfo):

    def __init__(self, lang="zh", level=1, mark=""):
        pic_type = "word" if not TEST else "word_test"
        super(WordPictureInfo, self).__init__(
...
class WordBuilder(Builder):
    ...
    def __init__(self, queue, config):
        ...
        word_config = config["wordpicture"]
        super(WordBuilder, self).__init__(queue, word_config,
                                          selector=WordSelector,
                                          picture_info=WordPictureInfo)
    ...
    def _past_character(self, picture, elements):
        ...
        for character in characters:
            ...
            # print(np.asarray(character_img).shape, np.asarray(character_img).dtype)
